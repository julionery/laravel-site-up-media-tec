...
class PaginaTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $existe = Pagina::where('tipo', '=', 'sobre2')->count();

        if($existe)
        {
            $paginaSobre = Pagina::where('tipo', '=', 'sobre2')->first();
        }
        else
        {
            $paginaSobre = new Pagina();
        ...
        $paginaSobre->titulo = "Sobre 2";
        $paginaSobre->descricao = "Descrição sobre a empresa.";
        $paginaSobre->texto = "Texto sobre a empresa Os PrintF";
        $paginaSobre->tipo = "sobre2";

        $paginaSobre->save();

        echo "\nPaginaTableSeeder - Pagina sobre 2 criada com sucesso!";


        $existe = Pagina::where('tipo', '=', 'noticias')->count();

        if($existe)
        {
            $paginaNoticia = Pagina::where('tipo', '=', 'noticias')->first();
        }
        else
        {
            $paginaNoticia = new Pagina();
        }

        $paginaNoticia->titulo = "Nossas noticias";
        $paginaNoticia->descricao = "Nossa equipe sempre disposta a fazer o melhor por você!";
        $paginaNoticia->texto = "Texto adicional sobre a equipe";
        $paginaNoticia->tipo = "equipe";
        $paginaNoticia->save();
